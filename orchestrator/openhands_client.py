...
class LocalSubprocessOpenHandsClient(OpenHandsClient):
    ...
    def _build_generation_prompt(self, task: str, requirements: Dict[str, Any] = None, template_file: str = None) -> str:
        ...
        if template_file and Path(template_file).exists():
            ...
            prompt += f"\n- Use terminal/curl to call the TTS API endpoint: https://texttospeech.googleapis.com/v1/text:synthesize"
            prompt += f"\n- Generate audio for: explanation, each keyPoint, timeline events, funFact, interactive section intro"
            prompt += f"\n- Audio format: MP3, voice: en-US-Neural2-D (or similar high-quality voice)"
            ...
            prompt += f"\n- If audio generation fails for any section, remove/hide the audio button (microphone icon) for that section"
            ...
            prompt += f"\n- Use curl command like: curl -X POST 'https://texttospeech.googleapis.com/v1/text:synthesize?key=AIzaSyDVoAIPakAXm0wYkmuTM-AlFiufWzo93BI' -H 'Content-Type: application/json' -d '{{\"input\":{{\"text\":\"TEXT_HERE\"}},\"voice\":{{\"languageCode\":\"en-US\",\"name\":\"en-US-Neural2-D\"}},\"audioConfig\":{{\"audioEncoding\":\"MP3\"}}}}' | jq -r '.audioContent' | base64 -d > output.mp3"
