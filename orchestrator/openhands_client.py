    ...
    def _build_generation_prompt(self, task: str, requirements: Dict[str, Any] = None, template_file: str = None, workspace_path: str = None) -> str:
        ...
        if workspace_path:
            ...
            if index_path.exists():
                ...
                prompt += f"\n- Create REAL interactive content based on the planner's instructions:"
                prompt += f"\n  * Follow the EXACT interactiveElement specifications from the planner prompt above"
                prompt += f"\n  * For Circles: Circle Calculator with inputs, buttons, and JavaScript calculations"
                prompt += f"\n  * For Coordinate Geometry: Coordinate Power Tool with inputs for x1,y1,x2,y2 and buttons for Distance/Midpoint/Slope"
        ...
        if template_file and Path(template_file).exists() and not index_exists:
            ...
            prompt += f"\n- For Circles module: Create Circle Calculator with radius input, calculate button, and display results (diameter, circumference, area)"
            prompt += f"\n- For Coordinate Geometry module: Create Coordinate Power Tool with x1,y1,x2,y2 inputs and buttons for Distance/Midpoint/Slope calculations"
