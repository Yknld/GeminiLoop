    ...
    def _build_generation_prompt(self, task: str, requirements: Dict[str, Any] = None, template_file: str = None, workspace_path: str = None) -> str:
        ...
        if workspace_path:
            ...
            if index_path.exists():
                ...
                prompt += f"\n\n**NAVIGATION BUTTONS LOGIC (IMPORTANT):**"
                prompt += f"\n- The template has prev/next buttons that are automatically enabled/disabled based on module count"
                prompt += f"\n- prevButton is disabled when currentModuleIndex === 0 (first module)"
                prompt += f"\n- nextButton is disabled when currentModuleIndex === modules.length - 1 (last module)"
                prompt += f"\n- If there's only 1 module, nextButton will be disabled - this is CORRECT behavior, not an error"
                prompt += f"\n- DO NOT try to enable disabled buttons or modify this logic - it works correctly"
                prompt += f"\n- The buttons are greyed out when disabled - this is intentional UI feedback"
